...
public class SimpleData extends DataA {
	...
	@Override
	public int calculateDaysSinceLast(CrimeKey key, int day) {
		TreeMap<Integer,Integer> events = counts.get(key);
		if (events == null) { return day;} // no events
		Integer lastday = events.floorKey(day); 
		if (lastday == null) {return day;} // no events prior to or on this day
		return day - lastday; // otherwise, no of days since last event.
	}
